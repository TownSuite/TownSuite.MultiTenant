...
function nuget_restore()
{
	if ($env:NUGET_CONFIG_FILE) {
		...
		dotnet restore "TownSuite.MultiTenant.sln" --configfile "$env:NUGET_CONFIG_FILE"
	}
	else {
		Write-Output "Using default NuGet.Config file"
		dotnet restore "TownSuite.MultiTenant.sln"
	}
}

function build()
{
	Write-Host "build TownSuite.MultiTenant.sln" -ForegroundColor Green
	& dotnet build -c Release TownSuite.MultiTenant.sln -p:GeneratePackageOnBuild=false
	if ($LastExitCode -ne 0) { throw "Building solution, TownSuite.MultiTenant.sln, failed" }
}
